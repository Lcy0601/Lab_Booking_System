  					...
  					item.backMenu.forEach(item1=>{
						if(item1.child&&item1.child.length>1 ) {
							item1.child.forEach(item2=>{
								if(item2.tableName=='shebeiyuyue') {
									if(
										...
										arr.push({
											...
											buttons: ['查看'],
											menu: '设备预约统计',
											tableName: 'shebeiyuyuestat',
										})
									}
								}
							})
						}else if(item1.child&&item1.child.length==1 ) {
							if(item1.menu == '设备预约管理') {
								if(
									this.isAuth('shebeiyuyue','学生预约次数')
									||this.isAuth('shebeiyuyue','院系预约次数')
									||this.isAuth('shebeiyuyue','设备预约次数')
