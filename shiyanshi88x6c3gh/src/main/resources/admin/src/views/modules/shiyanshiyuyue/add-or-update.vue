	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="预约时间" prop="appointmentdate" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.appointmentdate" label="预约时间" prop="appointmentdate" >
					<el-input v-model="ruleForm.appointmentdate" placeholder="预约时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="时间段" prop="timeperiod" >
					<el-select :disabled="ro.timeperiod" v-model="ruleForm.timeperiod" placeholder="请选择时间段"  filterable>
						<el-option
				...
				<el-form-item v-else class="input" label="时间段" prop="timeperiod" >
					<el-input v-model="ruleForm.timeperiod"
						placeholder="时间段" readonly></el-input>
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'"  label="预约状态" prop="yuyuezhuangtai" >
					<el-select :disabled="ro.yuyuezhuangtai" v-model="ruleForm.yuyuezhuangtai" placeholder="请选择预约状态"  filterable>
						<el-option
				...
				<el-form-item v-else class="input" label="预约状态" prop="yuyuezhuangtai" >
					<el-input v-model="ruleForm.yuyuezhuangtai"
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					this.ruleForm.yuyuezhuangtai = '待签到'; 				}
				...
				this.yuyuezhuangtaiOptions = "已签到,已取消,待签到".split(',')
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							let type = 0
							await this.$http.get('shiyanshiyuyue/list', {
								params: {
									// 标题设置了吗
									shiyanshimingcheng: this.ruleForm.shiyanshimingcheng,
									appointmentdatestart: this.ruleForm.appointmentdate,
									appointmentdateend: this.ruleForm.appointmentdate,
								...
								if(rs.data.data.list.length) {
									...
									if(obj.reduciblenum) {
										if(rs.data.data.list.length>=obj.reduciblenum) {
											type = 2
										}
									}else {
										type = 1
							...
							if(type==1) {
								this.$message.error(`该时间已被预约`)
								return false
							}else if(type==2) {
								this.$message.error(`该时间可预约次数已达上限`)
								return false
							}
							if(this.type=='cross'){
