...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="提问" prop="ask" >
					<el-input v-model="ruleForm.ask" placeholder="提问" clearable  :readonly="ro.ask"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="提问" prop="ask" >
					<el-input v-model="ruleForm.ask" placeholder="提问" readonly></el-input>
				</el-form-item>
			</template>
			...
			<el-form-item v-else-if="ruleForm.reply" label="回复" prop="reply"  class="textBox">
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `chathelper/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chathelperCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
