	...
	<div class="main-content" :style='{"width":"100%","padding":"10px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"20px 10px 10px","boxShadow":"none","borderRadius":"4px 4px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 18px 0 8px","outline":"none","margin":"0","color":"#fff","borderRadius":"4px","background":"#017cfe","width":"auto","fontSize":"inherit","height":"36px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0 10px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% + 0px)"}'>
					...
					<el-button class="add" v-if="isAuth('discussshiyanzhidao','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('discussshiyanzhidao','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 10px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussshiyanzhidao','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai13" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								编辑
							</el-button>

							<el-button class="view" v-if="isAuth('discussshiyanzhidao','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shiyanzhidao','删除评论')" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
	...
	export default {
		...
		methods: {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussdiscussshiyanzhidao',query:{refid:id}});
			},
			backClick(){
				history.back()
			},
			discussistopChange(e,row){
				this.$http({
					url:'discussshiyanzhidao/update',
					method:'post',
					data:row
				}).then(res=>{})
			},
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussshiyanzhidao/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'shiyanzhidao/info/' + refid,
								method: "get",
							}).then(res=>{
								if(res.data&&res.data.code==0){
									res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
									this.$http({
										url: 'shiyanzhidao/update',
										method: "post",
										data:res.data.data
									}).then(res1=>{
