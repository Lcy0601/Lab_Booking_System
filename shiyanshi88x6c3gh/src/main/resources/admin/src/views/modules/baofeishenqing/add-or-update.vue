...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'" label="报废编号" prop="baofeibianhao" >
					<el-input v-model="ruleForm.baofeibianhao" placeholder="报废编号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.baofeibianhao" label="报废编号" prop="baofeibianhao" >
					<el-input v-model="ruleForm.baofeibianhao" placeholder="报废编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item v-else class="input" label="设备编号" prop="shebeibianhao" >
				...
				<el-form-item v-else class="input" label="设备分类" prop="shebeifenlei" >
					<el-input v-model="ruleForm.shebeifenlei" placeholder="设备分类" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="报废数量" prop="shuliang" >
					<el-input v-model.number="ruleForm.shuliang" placeholder="报废数量" clearable  :readonly="ro.shuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="报废数量" prop="shuliang" >
					<el-input v-model="ruleForm.shuliang" placeholder="报废数量" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="date" v-if="type!='info'" label="报废时间" prop="baofeishijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.baofeishijian" label="报废时间" prop="baofeishijian" >
					<el-input v-model="ruleForm.baofeishijian" placeholder="报废时间" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="报废说明" prop="baofeishuoming" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="报废说明"
					v-model="ruleForm.baofeishuoming" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.baofeishuoming" label="报废说明" prop="baofeishuoming"  class="textBox">
				<span class="text">{{ruleForm.baofeishuoming}}</span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		created() {
			this.ruleForm.baofeishijian = this.getCurDateTime()
		},
		methods: {
			imgPreView(url){
			...
			async onSubmit() {
					...
					if(objcross!=null) {
						if(!this.ruleForm.shuliang&&this.ruleForm.shuliang!=0){
							this.$message.error("报废数量不能为空");
							return
						}
						if(objcross.shuliang<this.ruleForm.shuliang){
							this.$message.error("报废数量不足");
							return
						}
					}
					if(!this.ruleForm.id) {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							let errMsg = ''
							await this.$http({
								url: `baofeishenqing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.baofeishenqingCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
