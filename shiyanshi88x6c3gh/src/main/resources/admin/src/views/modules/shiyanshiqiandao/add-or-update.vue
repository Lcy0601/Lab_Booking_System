	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="签退状态" prop="qiantuizhuangtai" >
					<el-select :disabled="ro.qiantuizhuangtai" v-model="ruleForm.qiantuizhuangtai" placeholder="请选择签退状态"  filterable>
						<el-option
				...
				<el-form-item v-else class="input" label="签退状态" prop="qiantuizhuangtai" >
					<el-input v-model="ruleForm.qiantuizhuangtai"
						placeholder="签退状态" readonly></el-input>
				</el-form-item>
			</template>
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					this.ruleForm.qiantuizhuangtai = '待签退'; 				}
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shiyanshiqiandao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shiyanshiqiandaoCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
