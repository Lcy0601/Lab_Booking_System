	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="实验环境" prop="shiyanhuanjing" >
					<el-select :disabled="ro.shiyanhuanjing" v-model="ruleForm.shiyanhuanjing" placeholder="请选择实验环境"  filterable>
						<el-option
							v-for="(item,index) in shiyanhuanjingOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="实验环境" prop="shiyanhuanjing" >
					<el-input v-model="ruleForm.shiyanhuanjing"
						placeholder="实验环境" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shiyanshipingjia/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shiyanshipingjiaCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
