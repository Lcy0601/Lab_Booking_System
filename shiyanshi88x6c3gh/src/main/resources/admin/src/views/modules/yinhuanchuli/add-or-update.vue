	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-else-if="ruleForm.shangbaoshijian" label="上报时间" prop="shangbaoshijian" >
					<el-input v-model="ruleForm.shangbaoshijian" placeholder="上报时间" readonly></el-input>
				</el-form-item>
				<el-form-item class="upload" v-if="type!='info' && !ro.shipaizhaopian" label="实拍照片" prop="shipaizhaopian" >
					<file-upload
						tip="点击上传实拍照片"
						action="file/upload"
						:limit="3"
						:disabled="ro.shipaizhaopian"
						:multiple="true"
						:fileUrls="ruleForm.shipaizhaopian?ruleForm.shipaizhaopian:''"
						@change="shipaizhaopianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item class="upload" v-else-if="ruleForm.shipaizhaopian" label="实拍照片" prop="shipaizhaopian" >
					<img v-if="ruleForm.shipaizhaopian.substring(0,4)=='http'&&ruleForm.shipaizhaopian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.shipaizhaopian" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.shipaizhaopian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.shipaizhaopian.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shipaizhaopian.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.chulishijian" label="处理时间" prop="chulishijian" >
			...
			<el-form-item v-if="type!='info'" class="editorBox" label="安全隐患" prop="anquanyinhuan" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.anquanyinhuan" 
					class="editor"
					myQuillEditor="anquanyinhuan"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.anquanyinhuan" label="安全隐患" prop="anquanyinhuan"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.anquanyinhuan"></span>
			</el-form-item>
			<el-form-item v-if="type!='info'" class="editorBox" label="隐患处理" prop="yinhuanchuli" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.yinhuanchuli" 
					class="editor"
					myQuillEditor="yinhuanchuli"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yinhuanchuli" label="隐患处理" prop="yinhuanchuli"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.yinhuanchuli"></span>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='shipaizhaopian'){
							this.ruleForm.shipaizhaopian = obj[o];
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.anquanyinhuan = this.ruleForm.anquanyinhuan.replace(reg,'../../../shiyanshi88x6c3gh/upload');
						this.ruleForm.yinhuanchuli = this.ruleForm.yinhuanchuli.replace(reg,'../../../shiyanshi88x6c3gh/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			// 提交
			async onSubmit() {
					if(this.ruleForm.shipaizhaopian!=null) {
						this.ruleForm.shipaizhaopian = this.ruleForm.shipaizhaopian.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yinhuanchuli/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yinhuanchuliCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
			...
			shipaizhaopianUploadChange(fileUrls) {
				this.ruleForm.shipaizhaopian = fileUrls;
			},
		}
	};
</script>
