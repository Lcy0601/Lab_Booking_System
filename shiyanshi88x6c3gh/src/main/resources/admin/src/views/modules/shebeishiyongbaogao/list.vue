		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0 10px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% + 0px)"}'>
					<el-button class="add" v-if="isAuth('shebeishiyongbaogao','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shebeishiyongbaogao','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						批量删除
					</el-button>


					<download-excel v-if="isAuth('shebeishiyongbaogao','导出')" class="export-excel-wrapper" :fetch="getAllList" :fields="json_fields" name="设备使用报告.xls">
						<!-- 导出excel -->
			...
			<div :style='{"width":"100%","padding":"10px 10px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('shebeishiyongbaogao','查看')"
					:data="dataList"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shebeishiyongbaogao','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('shebeishiyongbaogao','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shebeishiyongbaogao','删除')" type="primary" @click="deleteHandler(scope.row.id)">
...
//导出excel
				json_fields: {
					...
					"封面": "fengmian",    //常规字段
					"型号": "xinghao",    //常规字段
					"上报时间": "shangbaoshijian",    //常规字段
					"学生学号": "xueshengxuehao",    //常规字段
					"学生姓名": "xueshengxingming",    //常规字段
					"手机号": "shoujihao",    //常规字段
					"使用情况": "shiyongqingkuang",    //常规字段
					"耗损反馈": "haosunfankui",    //常规字段
				},
				json_meta: [
					[
						{
		...
		methods: {
			...
			async deleteHandler(id ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "shebeishiyongbaogao/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
