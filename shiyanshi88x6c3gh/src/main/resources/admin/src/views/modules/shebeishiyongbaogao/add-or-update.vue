...
<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="使用情况" prop="shiyongqingkuang" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="使用情况"
					v-model="ruleForm.shiyongqingkuang" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.shiyongqingkuang" label="使用情况" prop="shiyongqingkuang"  class="textBox">
				<span class="text">{{ruleForm.shiyongqingkuang}}</span>
			</el-form-item>
			<el-form-item class="textarea" v-if="type!='info'" label="耗损反馈" prop="haosunfankui" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="耗损反馈"
					v-model="ruleForm.haosunfankui" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.haosunfankui" label="耗损反馈" prop="haosunfankui"  class="textBox">
				<span class="text">{{ruleForm.haosunfankui}}</span>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			async info(id) {
				await this.$http({
					url: `shebeishiyongbaogao/info/${id}`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shebeishiyongbaogao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shebeishiyongbaogaoCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
