...
<template>
	<div class="main-content" :style='{"width":"100%","padding":"10px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0 10px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% + 0px)"}'>
					<el-button class="add" v-if="isAuth('shiyanshiqiantui','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shiyanshiqiantui','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 10px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('shiyanshiqiantui','查看')"
					:data="dataList"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shiyanshiqiantui','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('shiyanshiqiantui','实验室评价')" @click="shiyanshipingjiaCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								实验室评价
							</el-button>
							<el-button class="btn8" v-if="isAuth('shiyanshiqiantui','安全隐患')" @click="anquanyinhuanCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								安全隐患
							</el-button>
							<el-button class="edit" v-if=" isAuth('shiyanshiqiantui','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shiyanshiqiantui','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<shiyanshipingjia-cross-add-or-update v-if="shiyanshipingjiaCrossAddOrUpdateFlag" :parent="this" ref="shiyanshipingjiaCrossaddOrUpdate"></shiyanshipingjia-cross-add-or-update>
		<anquanyinhuan-cross-add-or-update v-if="anquanyinhuanCrossAddOrUpdateFlag" :parent="this" ref="anquanyinhuanCrossaddOrUpdate"></anquanyinhuan-cross-add-or-update>
...
<script>
	...
	import shiyanshipingjiaCrossAddOrUpdate from "../shiyanshipingjia/add-or-update";
	import anquanyinhuanCrossAddOrUpdate from "../anquanyinhuan/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				shiyanshipingjiaCrossAddOrUpdateFlag: false,
				anquanyinhuanCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			shiyanshipingjiaCrossAddOrUpdate,
			anquanyinhuanCrossAddOrUpdate,
		},
		methods: {
			queryChange(arr){
				for(let x in arr) {
			...
			shiyanshipingjiaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shiyanshipingjiaCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shiyanshiqiantui');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shiyanshipingjiaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shiyanshipingjiaCrossaddOrUpdate.init(row.id,type);
				});
			},
			anquanyinhuanCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.anquanyinhuanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shiyanshiqiantui');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.anquanyinhuanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.anquanyinhuanCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
			},
