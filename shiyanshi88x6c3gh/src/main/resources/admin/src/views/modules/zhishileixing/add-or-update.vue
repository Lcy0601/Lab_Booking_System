...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="知识类型" prop="zhishileixing" >
					<el-input v-model="ruleForm.zhishileixing" placeholder="知识类型" clearable  :readonly="ro.zhishileixing"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="知识类型" prop="zhishileixing" >
					<el-input v-model="ruleForm.zhishileixing" placeholder="知识类型" readonly></el-input>
				</el-form-item>
			</template>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `zhishileixing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zhishileixingCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
