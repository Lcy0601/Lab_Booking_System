...
<template>
	<div class="main-content" :style='{"width":"100%","padding":"10px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"20px 10px 10px","boxShadow":"none","borderRadius":"4px 4px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					...
					<div v-if="queryChange(['管理员'])" :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">学生姓名</label>
						<el-input v-model="searchForm.xueshengxingming" placeholder="学生姓名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">预约时间起始</label>
						<el-date-picker v-model="searchForm.appointmentdatestart" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="预约时间起始"></el-date-picker>
					</div>
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">预约时间结束</label>
						<el-date-picker v-model="searchForm.appointmentdateend" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="预约时间结束"></el-date-picker>
					</div>
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}' class="select" label="时间段" prop="timeperiod">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">时间段</label>
						<el-select clearable v-model="searchForm.timeperiod" placeholder="请选择时间段" >
							<el-option v-for="(item,index) in timeperiodOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}' class="select">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0 10px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% + 0px)"}'>
					<el-button class="add" v-if="isAuth('shiyanshiyuyue','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shiyanshiyuyue','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						批量删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('shiyanshiyuyue','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe9" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						审核
					</el-button>
					<el-button class="btn18" v-if="isAuth('shiyanshiyuyue','实验室预约次数')" type="success" @click="chartDialogShow1">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						实验室预约次数
					</el-button>
					<el-button class="btn18" v-if="isAuth('shiyanshiyuyue','实验室类型预约次数')" type="success" @click="chartDialogShow2">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						实验室类型预约次数
					</el-button>
					<el-button class="btn18" v-if="isAuth('shiyanshiyuyue','日实验室预约次数')" type="success" @click="chartDialogShow3">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						日实验室预约次数
					</el-button>
					<el-button class="btn18" v-if="isAuth('shiyanshiyuyue','院系预约实验室次数')" type="success" @click="chartDialogShow4">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						院系预约实验室次数
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"10px 10px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shiyanshiyuyue','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('shiyanshiyuyue','签到')" @click="shiyanshiqiandaoCrossAddOrUpdateHandler(scope.row,'cross','是','','yuyuezhuangtai','已签到','已签到,已取消,待签到'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								签到
							</el-button>
							<el-button class="btn8" v-if="isAuth('shiyanshiyuyue','取消预约')" @click="quxiaoshiyanshiyuyueCrossAddOrUpdateHandler(scope.row,'cross','是','','yuyuezhuangtai','已签到不能取消','已签到,已取消,待签到'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								取消预约
							</el-button>
							<el-button class="edit" v-if=" isAuth('shiyanshiyuyue','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shiyanshiyuyue','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<shiyanshiqiandao-cross-add-or-update v-if="shiyanshiqiandaoCrossAddOrUpdateFlag" :parent="this" ref="shiyanshiqiandaoCrossaddOrUpdate"></shiyanshiqiandao-cross-add-or-update>
		<quxiaoshiyanshiyuyue-cross-add-or-update v-if="quxiaoshiyanshiyuyueCrossAddOrUpdateFlag" :parent="this" ref="quxiaoshiyanshiyuyueCrossaddOrUpdate"></quxiaoshiyanshiyuyue-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="shiyanshimingchengChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="shiyanshileixingChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="appointmentdateChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="yuanxiChart4" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import shiyanshiqiandaoCrossAddOrUpdate from "../shiyanshiqiandao/add-or-update";
	import quxiaoshiyanshiyuyueCrossAddOrUpdate from "../quxiaoshiyanshiyuyue/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				shiyanshiqiandaoCrossAddOrUpdateFlag: false,
				quxiaoshiyanshiyuyueCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			shiyanshiqiandaoCrossAddOrUpdate,
			quxiaoshiyanshiyuyueCrossAddOrUpdate,
		},
		methods: {
			queryChange(arr){
				for(let x in arr) {
			...
			shiyanshiqiandaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.shiyanshiqiandaoCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shiyanshiyuyue');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shiyanshiqiandaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shiyanshiqiandaoCrossaddOrUpdate.init(row.id,type);
				});
			},
			quxiaoshiyanshiyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "warning",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.quxiaoshiyanshiyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shiyanshiyuyue');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.quxiaoshiyanshiyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.quxiaoshiyanshiyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var shiyanshimingchengChart1 = echarts.init(document.getElementById("shiyanshimingchengChart1"),'macarons');
					let params = {}
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							shiyanshimingchengChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shiyanshimingchengChart1.resize();
							};
						}
					})
				})
			...
			chartDialog2() {
				this.$nextTick(()=>{
					var shiyanshileixingChart2 = echarts.init(document.getElementById("shiyanshileixingChart2"),'macarons');
					let params = {}
					if(params.conditionColumn) {
						params.conditionColumn += ';' + 'sfsh'
						params.conditionValue += ';' + '是'
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							shiyanshileixingChart2.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shiyanshileixingChart2.resize();
							};
						}
					})
				})
			...
			chartDialog3() {
				this.$nextTick(()=>{
					var appointmentdateChart3 = echarts.init(document.getElementById("appointmentdateChart3"),'macarons');
					let params = {}
					if(params.conditionColumn) {
						params.conditionColumn += ';' + 'sfsh'
						params.conditionValue += ';' + '是'
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							appointmentdateChart3.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								appointmentdateChart3.resize();
							};
						}
					})
				})
			...
			chartDialog4() {
				this.$nextTick(()=>{
					var yuanxiChart4 = echarts.init(document.getElementById("yuanxiChart4"),'macarons');
					let params = {}
					if(params.conditionColumn) {
						params.conditionColumn += ';' + 'sfsh'
						params.conditionValue += ';' + '是'
			...
			getDataList() {
				...
				if(this.searchForm.appointmentdatestart!='' && this.searchForm.appointmentdatestart!=undefined ){
					params['appointmentdatestart'] = this.searchForm.appointmentdatestart
				}
				if(this.searchForm.appointmentdateend!='' && this.searchForm.appointmentdateend!=undefined){
					params['appointmentdateend'] = this.searchForm.appointmentdateend
				}
				if(this.searchForm.timeperiod!='' && this.searchForm.timeperiod!=undefined){
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "shiyanshiyuyue/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									if(this.shBatchForm.sfsh!='待审核'){
										for(let x in this.shList){
											await this.$http.get('xuesheng/query',{params: {
												xueshengxuehao: this.shList[x].xueshengxuehao
											}}).then(async rs=>{
												if(rs.data.data.email) {
													await this.$http.get('xuesheng/sendemail/content',{params: {
														email: rs.data.data.email,
														title: '实验室预约',
														content: `你提交的“实验室预约”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
													}}).then(rs=>{})
												}
												let param = {
