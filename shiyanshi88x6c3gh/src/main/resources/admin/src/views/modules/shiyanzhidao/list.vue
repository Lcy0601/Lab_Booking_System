		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"20px 10px 10px","boxShadow":"none","borderRadius":"4px 4px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">知识标题</label>
						<el-input v-model="searchForm.zhishibiaoti" placeholder="知识标题" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}' class="select" label="知识类型" prop="zhishileixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">知识类型</label>
						<el-select clearable v-model="searchForm.zhishileixing" placeholder="请选择知识类型" >
							<el-option v-for="(item,index) in zhishileixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0 10px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% + 0px)"}'>
					<el-button class="add" v-if="isAuth('shiyanzhidao','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shiyanzhidao','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 10px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' prop="caozuoshipin" label="操作视频">
						<template slot-scope="scope">
							<el-button v-if="scope.row.caozuoshipin" type="text" size="small" @click="preClick(scope.row.caozuoshipin)">预览</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shiyanzhidao','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('shiyanzhidao','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai13" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								编辑
							</el-button>

							<el-button class="view" v-if="isAuth('shiyanzhidao','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('shiyanzhidao','删除')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.zhishibiaoti!='' && this.searchForm.zhishibiaoti!=undefined){
					params['zhishibiaoti'] = '%' + this.searchForm.zhishibiaoti + '%'
				}
				if(this.searchForm.zhishileixing!='' && this.searchForm.zhishileixing!=undefined){
					params['zhishileixing'] = this.searchForm.zhishileixing
				}
				let user = JSON.parse(this.$storage.getObj('userForm'))
				this.$http({
					url: "shiyanzhidao/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussshiyanzhidao',query:{refid:id}});
			},
			// 预览
			preClick(file){
				if(!file){
