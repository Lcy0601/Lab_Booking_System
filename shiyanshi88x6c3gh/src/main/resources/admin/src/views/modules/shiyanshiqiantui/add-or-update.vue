	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="date" v-if="type!='info'" label="签退时间" prop="qiantuishijian" >
					<el-date-picker
				...
				<el-form-item class="input" v-else-if="ruleForm.qiantuishijian" label="签退时间" prop="qiantuishijian" >
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `shiyanshiqiantui/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.shiyanshiqiantuiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
