		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="院系" prop="yuanxi" >
					<el-select :disabled="ro.yuanxi" v-model="ruleForm.yuanxi" placeholder="请选择院系"  filterable>
						<el-option
				...
				<el-form-item class="upload" v-else-if="ruleForm.xianchangzhaopian" label="现场照片" prop="xianchangzhaopian" >
					<img v-if="ruleForm.xianchangzhaopian.substring(0,4)=='http'&&ruleForm.xianchangzhaopian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.xianchangzhaopian" width="100" height="100" @click="imgPreView(ruleForm.tupian)">
					<img v-else-if="ruleForm.xianchangzhaopian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.xianchangzhaopian.split(',')[0]" width="100" height="100" @click="imgPreView(ruleForm.tupian.split(',')[0])">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xianchangzhaopian.split(',')" :src="$base.url+item" width="100" height="100" @click="imgPreView($base.url+item)">
				</el-form-item>
				<el-form-item class="date" v-if="type!='info'" label="巡查时间" prop="xunchashijian" >
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.xunchashijian" 
						type="datetime"
						:readonly="ro.xunchashijian"
						placeholder="巡查时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.xunchashijian" label="巡查时间" prop="xunchashijian" >
					<el-input v-model="ruleForm.xunchashijian" placeholder="巡查时间" readonly></el-input>
				</el-form-item>
			</template>
			<el-form-item class="textarea" v-if="type!='info'" label="仪器配置" prop="yiqipeizhi" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="仪器配置"
					v-model="ruleForm.yiqipeizhi" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yiqipeizhi" label="仪器配置" prop="yiqipeizhi"  class="textBox">
				<span class="text">{{ruleForm.yiqipeizhi}}</span>
			</el-form-item>
			<el-form-item class="textarea" v-if="type!='info'" label="巡查项目" prop="xunchaxiangmu" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="巡查项目"
					v-model="ruleForm.xunchaxiangmu" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.xunchaxiangmu" label="巡查项目" prop="xunchaxiangmu"  class="textBox">
				<span class="text">{{ruleForm.xunchaxiangmu}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'" class="editorBox" label="巡查记录" prop="xunchajilu" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.xunchajilu" 
					class="editor"
					myQuillEditor="xunchajilu"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.xunchajilu" label="巡查记录" prop="xunchajilu"  class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.xunchajilu"></span>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			this.ruleForm.xunchashijian = this.getCurDateTime()
		},
		methods: {
			imgPreView(url){
			...
			init(id,type ) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='yiqipeizhi'){
							this.ruleForm.yiqipeizhi = obj[o];
							this.ro.yiqipeizhi = true;
							continue;
						}
						...
						if(o=='xunchaxiangmu'){
							this.ruleForm.xunchaxiangmu = obj[o];
							this.ro.xunchaxiangmu = true;
							continue;
						}
						if(o=='xunchashijian'){
							this.ruleForm.xunchashijian = obj[o];
							this.ro.xunchashijian = true;
							continue;
						}
						if(o=='xunchajilu'){
							this.ruleForm.xunchajilu = obj[o];
							this.ro.xunchajilu = true;
							continue;
						}
					}
				}
			...
			async info(id) {
				await this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.xunchajilu = this.ruleForm.xunchajilu.replace(reg,'../../../shiyanshi88x6c3gh/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `xunchajilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xunchajiluCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
