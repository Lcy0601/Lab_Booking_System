	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="借用数量" prop="shuliang" >
					<el-input v-model.number="ruleForm.shuliang" placeholder="借用数量" clearable  :readonly="ro.shuliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="借用数量" prop="shuliang" >
					<el-input v-model="ruleForm.shuliang" placeholder="借用数量" readonly></el-input>
				</el-form-item>
				...
				<el-form-item class="input" v-else-if="ruleForm.jieyongshijian" label="借用时间" prop="jieyongshijian" >
			...
			<el-form-item class="textarea" v-if="type!='info'" label="取消说明" prop="quxiaoshuoming" >
				<el-input
					style="min-width: 200px; max-width: 600px;"
					type="textarea"
					:rows="8"
					placeholder="取消说明"
					v-model="ruleForm.quxiaoshuoming" >
				</el-input>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.quxiaoshuoming" label="取消说明" prop="quxiaoshuoming"  class="textBox">
				<span class="text">{{ruleForm.quxiaoshuoming}}</span>
			</el-form-item>
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							if(this.type=='cross'){
								...
								if(statusColumnName!='') {
									...
									if(statusColumnName && !statusColumnName.startsWith("[")) {
										var notUpdateColumn = 'guihaizhuangtai,guihaizhuangtai'.split(',')
							...
							let errMsg = ''
							if(this.type=='cross') {
								await this.$http.post('updateColumn/shebeiyuyue/1',{
									csuUpdateColumn: 'guihaizhuangtai',
									csuUpdateColumnValue: '已归还',
							...
							await this.$http({
								url: `quxiaoshebeiyuyue/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.quxiaoshebeiyuyueCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
