...
<template>
	<div class="main-content" :style='{"width":"100%","padding":"10px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0 10px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% + 0px)"}'>
					<el-button class="add" v-if="isAuth('anquanyinhuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('anquanyinhuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 10px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column  :resizable='true' prop="shipaizhaopian" width="200" label="实拍照片">
						<template slot-scope="scope">
							<div v-if="scope.row.shipaizhaopian">
								<img v-if="scope.row.shipaizhaopian.substring(0,4)=='http'&&scope.row.shipaizhaopian.split(',w').length>1" :src="scope.row.shipaizhaopian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.shipaizhaopian)">
								<img v-else-if="scope.row.shipaizhaopian.substring(0,4)=='http'" :src="scope.row.shipaizhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.shipaizhaopian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.shipaizhaopian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.shipaizhaopian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('anquanyinhuan','审核')" prop="sfsh" label="回复">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('anquanyinhuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('anquanyinhuan','隐患处理')" @click="yinhuanchuliCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								隐患处理
							</el-button>
							<el-button class="edit" v-if=" isAuth('anquanyinhuan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('anquanyinhuan','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<yinhuanchuli-cross-add-or-update v-if="yinhuanchuliCrossAddOrUpdateFlag" :parent="this" ref="yinhuanchuliCrossaddOrUpdate"></yinhuanchuli-cross-add-or-update>
...
<script>
	...
	import yinhuanchuliCrossAddOrUpdate from "../yinhuanchuli/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				yinhuanchuliCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yinhuanchuliCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yinhuanchuliCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','anquanyinhuan');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yinhuanchuliCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yinhuanchuliCrossaddOrUpdate.init(row.id,type);
				});
			},
			init () {
			},
			...
			shDialog(row){
				...
				if(row){
					this.shForm = {
						...
						shiyanshileixing: row.shiyanshileixing,
						yuanxi: row.yuanxi,
						shiyanshiweizhi: row.shiyanshiweizhi,
						...
						shangbaoshijian: row.shangbaoshijian,
						anquanyinhuan: row.anquanyinhuan,
						shipaizhaopian: row.shipaizhaopian,
						shhf: row.shhf,
						id: row.id
					}
				}
