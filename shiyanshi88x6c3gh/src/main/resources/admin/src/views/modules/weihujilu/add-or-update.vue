...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'" label="维护编号" prop="weihubianhao" >
					<el-input v-model="ruleForm.weihubianhao" placeholder="维护编号" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.weihubianhao" label="维护编号" prop="weihubianhao" >
					<el-input v-model="ruleForm.weihubianhao" placeholder="维护编号" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.weihujilu" label="维护记录" prop="weihujilu"  class="textBox">
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `weihujilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.weihujiluCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
