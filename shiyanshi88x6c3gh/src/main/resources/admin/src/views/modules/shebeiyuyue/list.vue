...
<template>
	<div class="main-content" :style='{"width":"100%","padding":"10px","fontSize":"14px","color":"#666","height":"auto"}'>
			...
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"20px 10px 10px","boxShadow":"none","borderRadius":"4px 4px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative"}' >
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">设备名称</label>
						<el-input v-model="searchForm.shebeimingcheng" placeholder="设备名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">设备分类</label>
						<el-input v-model="searchForm.shebeifenlei" placeholder="设备分类" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"width":"auto","margin":"0 5px 0px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">型号</label>
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0 10px","margin":"0","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% + 0px)"}'>
					<el-button class="add" v-if="isAuth('shebeiyuyue','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('shebeiyuyue','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"16px","color":"inherit","display":"none","height":"40px"}'></span>
						批量删除
					</el-button>


					<download-excel v-if="isAuth('shebeiyuyue','导出')" class="export-excel-wrapper" :fetch="getAllList" :fields="json_fields" name="设备预约.xls">
						<!-- 导出excel -->
					...
					<el-button class="btn18" v-if="isAuth('shebeiyuyue','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"width":"100%","padding":"10px 10px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shebeiyuyue','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('shebeiyuyue','归还')" @click="shebeiguihaiCrossAddOrUpdateHandler(scope.row,'cross','是','','guihaizhuangtai','已归还','已归还,待归还'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								归还
							</el-button>
							<el-button class="btn8" v-if="isAuth('shebeiyuyue','取消')" @click="quxiaoshebeiyuyueCrossAddOrUpdateHandler(scope.row,'cross','是','','guihaizhuangtai','已归还不能取消','已归还,待归还'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								取消
							</el-button>
							<el-button class="edit" v-if=" isAuth('shebeiyuyue','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shebeiyuyue','删除')" type="primary" @click="deleteHandler(scope.row.id)">
		...
		<quxiaoshebeiyuyue-cross-add-or-update v-if="quxiaoshebeiyuyueCrossAddOrUpdateFlag" :parent="this" ref="quxiaoshebeiyuyueCrossaddOrUpdate"></quxiaoshebeiyuyue-cross-add-or-update>
		...
		<template v-if="statType">
			<div class="statShow4">
				<div class="echarts1" v-if="isAuth('shebeiyuyue','学生预约次数')">
					<div id="shuliangChart1" style=" width: 100%;height: 100%"></div>
				</div>
				<div class="echarts2" v-if="isAuth('shebeiyuyue','院系预约次数')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery2.yuanxi" placeholder="请选择" @change="chartDialog2" clearable>
						<el-option v-for="item in yuanxiChartOptions2" :label="item" :value="item"></el-option>
					</el-select>
					<div id="shuliangChart2" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
				<div class="echarts3" v-if="isAuth('shebeiyuyue','设备预约次数')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery3.shebeimingcheng" placeholder="请选择" @change="chartDialog3" clearable>
						<el-option v-for="item in shebeimingchengChartOptions3" :label="item" :value="item"></el-option>
					</el-select>
					<div id="shuliangChart3" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
				<div class="echarts4" v-if="isAuth('shebeiyuyue','设备归还占比')">
					<div id="shuliangChart4" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	import quxiaoshebeiyuyueCrossAddOrUpdate from "../quxiaoshebeiyuyue/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				shebeimingchengChartOptions3: [],
				chartQuery3: {},
				chartVisiable3: false,
				chartVisiable4: false,
				addOrUpdateFlag:false,
				shebeiguihaiCrossAddOrUpdateFlag: false,
				quxiaoshebeiyuyueCrossAddOrUpdateFlag: false,
				layouts: ["total","prev","pager","next","sizes","jumper"],
//导出excel
				json_fields: {
					"预约编号": "yuyuebianhao",    //常规字段
					"设备编号": "shebeibianhao",    //常规字段
					"设备名称": "shebeimingcheng",    //常规字段
					"设备分类": "shebeifenlei",    //常规字段
					"院系": "yuanxi",    //常规字段
					"借用数量": "shuliang",    //常规字段
					"封面": "fengmian",    //常规字段
					"型号": "xinghao",    //常规字段
					"借用时间": "jieyongshijian",    //常规字段
					"学生学号": "xueshengxuehao",    //常规字段
					"学生姓名": "xueshengxingming",    //常规字段
					"手机号": "shoujihao",    //常规字段
					"归还状态": "guihaizhuangtai",    //常规字段
					"用途": "yongtu",    //常规字段
					"是否审核": "sfsh",    //常规字段
					"审核回复": "shhf",    //常规字段
				},
		...
		components: {
			...
			quxiaoshebeiyuyueCrossAddOrUpdate,
		},
		methods: {
			statChartClick() {
				if(this.isAuth('shebeiyuyue','学生预约次数')) {
					this.chartDialogShow1()
				}
				if(this.isAuth('shebeiyuyue','院系预约次数')) {
					this.chartDialogShow2()
				}
				if(this.isAuth('shebeiyuyue','设备预约次数')) {
					this.chartDialogShow3()
				}
				if(this.isAuth('shebeiyuyue','设备归还占比')) {
					this.chartDialogShow4()
				}
			},
			queryChange(arr){
				for(let x in arr) {
			...
			shebeiguihaiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','shebeiyuyue');
			...
			quxiaoshebeiyuyueCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptAudit=='是'&&row.sfsh!='是') {
					this.$message({
						message: "请审核通过后再操作",
						type: "warning",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.quxiaoshebeiyuyueCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','shebeiyuyue');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.quxiaoshebeiyuyueCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.quxiaoshebeiyuyueCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialogShow2() {
				...
				this.$http.get('option/shebeiyuyue/yuanxi',
					...
					this.yuanxiChartOptions2 = rs.data.data
				})
				this.chartDialog2()
			},
			// 统计接口2
			chartDialog2() {
				this.$nextTick(()=>{
					...
					let params = {}
					if(this.chartQuery2.yuanxi) {
						params.conditionColumn = 'yuanxi'
						params.conditionValue = this.chartQuery2.yuanxi
					}
					if(params.conditionColumn) {
						params.conditionColumn += ';' + 'sfsh'
						params.conditionValue += ';' + '是'
			...
			chartDialogShow3() {
				...
				this.$http.get('option/shebeiyuyue/shebeimingcheng',
					...
					this.shebeimingchengChartOptions3 = rs.data.data
				})
				this.chartDialog3()
			},
			// 统计接口3
			chartDialog3() {
				this.$nextTick(()=>{
					var shuliangChart3 = echarts.init(document.getElementById("shuliangChart3"),'macarons');
					let params = {}
					if(this.chartQuery3.shebeimingcheng) {
						params.conditionColumn = 'shebeimingcheng'
						params.conditionValue = this.chartQuery3.shebeimingcheng
					}
					if(params.conditionColumn) {
						params.conditionColumn += ';' + 'sfsh'
						params.conditionValue += ';' + '是'
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							shuliangChart3.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shuliangChart3.resize();
							};
						}
					})
				})
			...
			chartDialog4() {
				this.$nextTick(()=>{
					var shuliangChart4 = echarts.init(document.getElementById("shuliangChart4"),'macarons');
					let params = {}
					if(params.conditionColumn) {
						params.conditionColumn += ';' + 'sfsh'
						params.conditionValue += ';' + '是'
					...
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							shuliangChart4.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shuliangChart4.resize();
							};
						}
					})
				})
						...
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							if(this.shBatchForm.sfsh=='是') {
								for(let x in this.shList){
									await this.$http.post('updateColumn/shiyanshebei/2',{
										csuUpdateColumn: 'shuliang',
										csuUpdateColumnValue: '-' + this.shList[x].shuliang,
							...
							this.$http({
								url: "shebeiyuyue/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
											...
											await this.$http.get('xuesheng/query',{params: {
												...
												if(rs.data.data.email) {
													await this.$http.get('xuesheng/sendemail/content',{params: {
														...
														title: '设备预约',
														content: `你提交的“设备预约”申请${this.shBatchForm.sfsh=='是'?'审核通过':'审核不通过'}`,
													}}).then(rs=>{})
												}
												let param = {
