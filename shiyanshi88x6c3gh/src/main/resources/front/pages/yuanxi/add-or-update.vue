	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yuanxi`, this.ruleForm.id);
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`yuanxi`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`yuanxi`, this.ruleForm);
				}else{
					oet = await this.$api.add(`yuanxi`, this.ruleForm);
				}
				if(this.cross){
