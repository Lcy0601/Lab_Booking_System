	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('shebeifenlei', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','shebeifenlei');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','shebeifenlei');
			...
			async init(type=1){
				...
				let res = await this.$api.info('shebeifenlei', this.id);
				let reg=new RegExp('http://localhost:8080/shiyanshi88x6c3gh/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('shebeifenlei',this.detail)
				}
			},
