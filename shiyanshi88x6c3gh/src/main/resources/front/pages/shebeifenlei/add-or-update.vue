	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`shebeifenlei`, this.ruleForm.id);
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`shebeifenlei`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`shebeifenlei`, this.ruleForm);
				}else{
					oet = await this.$api.add(`shebeifenlei`, this.ruleForm);
				}
				if(this.cross){
