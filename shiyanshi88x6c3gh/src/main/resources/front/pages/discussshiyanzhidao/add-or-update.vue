	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussshiyanzhidao`, this.ruleForm.id);
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`discussshiyanzhidao`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`discussshiyanzhidao`, this.ruleForm);
				}else{
					oet = await this.$api.add(`discussshiyanzhidao`, this.ruleForm);
				}
				uni.setStorageSync('discussshiyanzhidaoCleanType',true);
				if(this.cross){
