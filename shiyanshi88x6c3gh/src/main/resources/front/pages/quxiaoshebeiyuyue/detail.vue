	...
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			...
			<view :style='{"width":"100%","padding":"20rpx","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}' class="detail-content">
				...
				<view class="detail-list-item" :style='{"margin":"0","borderColor":"#ccc","borderWidth":"0","background":"#fff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"padding":"0 20rpx","whiteSpace":"nowrap","color":"#333","textAlign":"right","width":"auto","lineHeight":"48rpx","fontSize":"28rpx"}'>取消说明：</view>
					<view class="text" :style='{"padding":"0px","margin":"0px","lineHeight":"48rpx","fontSize":"28rpx","color":"#333333"}'>{{detail.quxiaoshuoming}}</view>
				</view>
				...
				<view class="bottom-content bg-white tabbar border shop" :style='{"padding":"10rpx 24rpx","flexWrap":"wrap","background":"#fff","display":"flex","gap":"20rpx","width":"100%","justifyContent":"center","height":"auto","order":"101"}'>
					...
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","alignItems":"center","color":"#298284","display":"flex","justifyContent":"center","outline":"none","borderRadius":"8rpx","background":"#29828430","width":"31%","fontSize":"28rpx","height":"80rpx"}' v-if="userid&&isAuth('quxiaoshebeiyuyue','审核')" @tap="onSHTap">审核</button>
					<button :style='{"border":"0","padding":"0 20rpx","margin":"0","alignItems":"center","color":"#298284","display":"flex","justifyContent":"center","outline":"none","borderRadius":"8rpx","background":"#29828430","width":"31%","fontSize":"28rpx","height":"80rpx"}' v-if="!userid&&isAuthFront('quxiaoshebeiyuyue','审核')" @tap="onSHTap">审核</button>
				</view>
			</view>
		</view>
		<!-- 确认完成弹窗 -->
	...
	export default {
		...
		async onShow(options) {
			...
			if(crossCleanType) {
				...
				res = await this.$api.info('quxiaoshebeiyuyue', this.id);
		...
		methods: {
			...
			onPayTap(){
				...
				uni.setStorageSync('paytable','quxiaoshebeiyuyue');
			...
			async onAcrossTap(tableName,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				uni.setStorageSync('crossTable','quxiaoshebeiyuyue');
			...
			async init(type=1){
				...
				let res = await this.$api.info('quxiaoshebeiyuyue', this.id);
				let reg=new RegExp('http://localhost:8080/shiyanshi88x6c3gh/upload','g')//g代表全部
				this.detail = res.data;
				...
				if(type==2){
					...
					await this.$api.update('quxiaoshebeiyuyue',this.detail)
				}
			},
				...
				if(this.detail.sfsh!="待审核"){
					...
					if(rs.data.email) {
						await this.$api.getPublic('xuesheng/sendemail/content',{
							email: rs.data.email,
							title: '取消设备预约',
							content: `你提交的“取消设备预约”申请${this.detail.sfsh=='是'?'审核通过':'审核不通过'}`,
						})
					}
