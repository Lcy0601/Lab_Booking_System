	...
	<view :style='{"minHeight":"100%","width":"100%","padding":"0","position":"relative","background":"#ECECEC","height":"auto"}' v-if="showType==1">
		<form :style='{"width":"100%","padding":"24rpx","background":"none","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#29828450","borderRadius":"60rpx","borderWidth":"2rpx","background":"#fff","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"100%","padding":"0 24rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","fontWeight":"500"}' class="title">评论内容</view>
				<xia-editor ref="content" :style='{"minHeight":"300rpx","border":"0px solid #efefef","padding":"12rpx 24rpx 24rpx 24rpx","borderRadius":"0 0 60rpx 60rpx","borderWidth":"2rpx 0 0 0","width":"100%","height":"auto"}' v-model="ruleForm.content" placeholder="评论内容" @editorChange="contentChange"></xia-editor>
			</view>
			<view :style='{"padding":"12rpx 0","margin":"0 0 24rpx 0","borderColor":"#29828450","borderRadius":"60rpx","borderWidth":"2rpx","background":"#fff","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"100%","padding":"0 24rpx","lineHeight":"80rpx","fontSize":"28rpx","color":"#333","fontWeight":"500"}' class="title">评分</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussshiyanshi`, this.ruleForm.id);
			...
			async onSubmitTap(subMitType=null) {
				...
				if(this.cross){
					...
					if(crossrefid && crossuserid) {
						...
						let res = await this.$api.list(`discussshiyanshi`, params);
				...
				let oet = {}
				if(this.ruleForm.id){
					await this.$api.update(`discussshiyanshi`, this.ruleForm);
				}else{
					oet = await this.$api.add(`discussshiyanshi`, this.ruleForm);
				}
				uni.setStorageSync('discussshiyanshiCleanType',true);
				let rs2 = await this.$api.list('discussshiyanshi',{
					page: 1,
					limit: 10000,
				...
				let rs3 = await this.$api.info('shiyanshi',this.ruleForm.refid)
				rs3.data.totalscore = rs2.data.list.length?Number((totalscore / rs2.data.list.length).toFixed(2)):0
				await this.$api.update('shiyanshi',rs3.data)
				if(this.cross){
